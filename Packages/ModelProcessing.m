...
(* Print[{"Upper bounds: counts and gaps ", hibins, counts, gap}]; *)
  ...
  scaletop = If[Total@counts == 0, 6, Log@Max[counts]];
...
Module[{tolerance, (* FBAtol,*) FBAflux, objective, optimum, Svals},
  	...
  	If[exempt,
  		exemptcount=Length@exemptions;
  		Print[ToString[exemptcount]<>" unbuffered externals are exempted from flux balance."];
  		MetaboliteNames=Delete[MetaboliteNames,Partition[exemptions,1]];
  		S=Delete[S,Partition[exemptions,1]],
  		exemptcount=0;	
  	];
...
(*  Print["maxmin,objectselector, S, Svals, bounds",maxmin," ",Length@objectselector, 
  	Dimensions@S, Dimensions@Svals, Dimensions@bounds]; *)
(*
...
*)     
     {LPtime, result} = Timing[ FBAflux = Check[
     LinearProgramming[maxmin*objectselector, S, Svals, bounds, Reals,
       Tolerance -> FBAtol, Method -> LPmethod], 
     "No LP solution was found! ",
...
KernelDisplay[exportdata_, Printresult_:False] := 
 Block[{exfeasibles, exrays, exSolutionSpace, exSSTransform, 
   ...
   exthindirs, exfixvals, fixdirnames, BaseDim, chordpic, cenpic, discrepancies, 
  ...
  *)
  ...
  fixdirnames=fixdirs;
  If[Length[fixdirs] > 0, 
   fixdirnames[[All,1]] = ReactionNames[[fixdirs[[All, 1]]]];
   Processreport = Join[Processreport,
   	{{"The following reactions acquired fixed directions:"},
     {TextGrid@Partition[fixdirnames,UpTo@5]}}]
     ];
  exfixvals = 
...
(* If the control window has a docked cell, everything below it is deleted.
   ...
   (* Species = If[Species == "" || StringMatchQ[Species, Whitespace], 
    ...
    {Sraw,rawSvals,rawbounds,rawobject,rawFBAvec,rawreacts,rawmets,rawexternals,rawexempts} = 
     	{S,Svals,bounds,objectselector,FBAvector,ReactionNames,MetaboliteNames,externals,exemptions}; 
  ...
  If[mainstage == "Reducing" || (automate && skipstages < 2),
   ...
   (* NOTE: The LP solver sometimes (!) gives "not a matrix" error message for sparse matrix, so 
   			convert to the normal form to prevemnt this.*)
   {S,Svals,bounds,objectselector,FBAvector,ReactionNames,MetaboliteNames,externals,exemptions} = 
    	{Normal@Sraw,rawSvals,rawbounds,rawobject,rawFBAvec,rawreacts,rawmets,rawexternals,rawexempts} ; 
 ...
 Module[{stage1, stage2, stage3, stage4, stage5, monitor, progress, 
  ...
  stage2 = 
   Panel[Grid[{{(*Optiontable[[15,1]],*)"Flux bounds larger", 
       "than this", "are taken as artificial:", 
       InputField[Dynamic[artificial], Number, 
        FieldSize -> 5]}, {Tooltip[Optiontable[[3, 1]], 
        "A single value used throughout, or a pair of values in \
braces; 
        the first is used for the initial bounded FBA only."], 
       InputField[Dynamic[Tolerances], String, FieldSize -> 5], 
       Optiontable[[4, 1]], 
       InputField[Dynamic[fixtol], Number, 
        FieldSize -> 3]}, {"Exempt unbuffered externals", 
       Checkbox[Dynamic[exempt, (FBAdone = False; exempt = #) &]], 
       "Reshuffle stoichiometry first", 
       Checkbox[
        Dynamic[reshuffle, (FBAdone = False; reshuffle = #) &]], 
       Button[Style[" Reduce ", 10, Bold], 
        StageManager["Reducing", reportwindow], ImageSize -> {80, 16},
         Background -> Lighter[Green, 0.95], 
        Enabled -> Dynamic@available[[2]], Method -> "Queued"]}}, 
     Spacings -> {2.0, 0}], 
    Style["Stage 2:   Reduction - Eliminate fixed fluxes and \
prismatic rays.", "Subsubtitle", FontSize -> 12],
    ImageSize -> {Automatic, 60}, 
    Background -> Dynamic[panelback[[2]]]];
  stage3 = 
   Panel[Grid[{{Optiontable[[5, 1]], 
       InputField[Dynamic[targetcount], Number, FieldSize -> 3], 
       Optiontable[[7, 1]], 
       InputField[Dynamic[samplesize], Number, 
        FieldSize -> 3]}, {Optiontable[[6, 1]], 
...
Export results.", "Subsubtitle", FontSize -> 12], 
  ...
  FontSize->12],Checkbox[Dynamic[verbose]],
  "                                                 ",Style[
  "Automate stages","Subsubtitle",FontSize->12],Checkbox[Dynamic[
  automate]]},Spacer[15]];
  Controlbox=Panel[Column[{stage1,stage2,stage3,stage4,stage5,monitor,
  progress},Spacer[20]],Background->LightGray(*,
  ImageSize\[Rule]Full*)];*)
  Controlbox = 
   Panel[Grid[{{stage1, Column[
       	{Panel[Checkbox[Dynamic[automate]], "Automate", Appearance -> "Frameless"], 
         Panel[Checkbox[Dynamic[verbose]], "Commentary",Appearance -> "Frameless"]}]}, 
       {stage2, Rotate[monitor, Pi/2]}, {stage3, SpanFromAbove}, 
       {stage4, SpanFromAbove}, {stage5, SpanFromAbove}}, Alignment -> Top, Spacings -> {1, 0.3}],
         Background -> LightGray, FrameMargins -> 1];
  StatusReport = 
   ExpressionCell[
        ...
        Column[{Style[Heading[[1]], "Subtitle"], 
          ...
          If[Length[Validations] > 0, 
           Labeled[Grid[{{Style["FULL\nSS KERNEL ", Bold], 
             ...
             Background -> {Automatic, {LightBlue, LightYellow}}, 
             Dividers -> {{{1}}, {{-1}, {1}}}, Alignment -> Left], 
            "\nVALIDATION TEST: Deconstruct FBA solutions \
(with/without artificial bounds) 
                	into the sum of a kernel flux, and a flux along a \
ray direction. 
                	Agreement between actual and reconstituted solutions \
are indicated by % discrepancy
                	of total flux, and angle in degrees between their \
directions in flux space.", Top], Null]}, Dividers -> Center, 
         Spacings -> 2], 
